...
class _HomeParentState extends State<HomeParent> {
  ...
  String getMissingVaccinesMessage() {
    final notCompleted = vaccinationThisPeriodList.where((item) => item['status'] == false).toList();

    if (notCompleted.isEmpty) {
      return "All vaccinations are completed for $childName.";
    }

    final missingNames = notCompleted.map((item) => item['name']).join(', ');

    return "$missingNames is missing for $childName";
  }
              ...
              children: [
                Column(
                    children: [
                      ...
                      //         body : getMissingVaccinesMessage(),
                      //       );
                      //     },
